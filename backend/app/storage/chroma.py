import chromadb 
from app.store.vector import Vector

class Chroma(Vector): 
    def __init__( self, embedding_service, collection_name="documents", persist_dir="./chroma_data" ): 
        self.embedding_service = embedding_service 
        self.client = chromadb.Client( 
            chromadb.config.Settings( persist_directory=persist_dir, is_persistent=True ) 
        ) 
        self.collection = self.client.get_or_create_collection( name=collection_name ) 

    def add_documents(self, ids, documents, metadata): 
        vectors = self.embedding_service.embed(documents) 
        self.collection.add( ids=ids, embeddings=vectors, documents=documents, metadatas=metadata ) 
